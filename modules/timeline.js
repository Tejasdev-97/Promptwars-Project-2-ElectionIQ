...
export const initTimeline = async () => {
  ...
  const loadData = async () => {
    ...
    // Try to fetch real-time data from Gemini if online
        ...
        if (resp.ok) {
          ...
          if (jsonStr) {
            ...
            if (aiData && aiData.elections) {
              data = aiData;
              localStorage.setItem('cached_timeline_data', JSON.stringify(data));
              console.log("Timeline data loaded from real-time Gemini API");
            }
          }
        }
      } catch (err) {
        console.warn("Failed to fetch real-time timeline from Gemini, falling back...", err);
      }
    }

    // Fallback to cache or local JSON
    if (!data) {
      const cached = localStorage.getItem('cached_timeline_data');
      if (cached) {
        data = JSON.parse(cached);
        console.log("Timeline data loaded from cache");
      } else {
        data = await fetchJSON('data/elections-india.json');
        console.log("Timeline data loaded from local JSON");
      }
    }
    ...
    if (data && data.elections) {
      ...
      // Clear select options and rebuild
      select.innerHTML = '';
      timelineData.forEach(elec => {
        const opt = document.createElement('option');
        opt.value = elec.id;
        opt.textContent = elec.name || elec.id;
          ...
          try {
            ...
            if (state) {
              ...
              if (stateIdMatch) {
                ...
                opt.textContent = `${state} Elections`;
                select.appendChild(opt);
